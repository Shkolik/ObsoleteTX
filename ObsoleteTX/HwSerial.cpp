 ...
 * HwSerial.cpp
 *
 * Created: 1/25/2020 12:19:22 AM
 *  Author: Andrew
 */ 

/*
Note: This HwSerial driver/module by RC-Navy is mainly based on the arduino HardwareSerial.
      This lightweight version allows dimensioning independently (statically) all the Tx/Rx buffers.
*/
...
#define DECLARE_SERIAL_TX_ISR(UartIdx) ISR(USART##UartIdx##_UDRE_vect) { SERIAL##UartIdx##_TX_ISR(); }
...
#define DECLARE_SERIAL_RX_ISR(UartIdx) ISR(USART##UartIdx##_RX_vect) { SERIAL##UartIdx##_RX_ISR(); }
...
void HwSerial::init(unsigned long baud, uint8_t config)
{
	...
	*ucsra = 1 << U2X0;
	...
	// hard coded exception for 57600 for compatibility with the boot loader
