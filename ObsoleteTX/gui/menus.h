...
/// return to last menu in menu stack
extern void popMenu();
...
extern int8_t s_editMode;       // global edit mode
...
#define CHECK_INCDEC_MODELVAR(event, var, min, max) var = checkIncDecModel(event,var,min,max)

#define CHECK_INCDEC_MODELVAR_STARTPULSES_IF_CHANGE(event, var, min, max) tmp = checkIncDecModel(event,var,min,max); if (tmp != var) { startPulses(PROTOCMD_INIT); }; var = tmp
