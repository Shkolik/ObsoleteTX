...
int16_t v = ((i < NUM_STICKS+NUM_POTS) ? analogIn(i) : getRotaryEncoder(i-(NUM_STICKS+NUM_POTS)));
#else
...
#define MIXER_LINE_DISABLE()  (mixCondition = true, mixEnabled = 0)
      ...
      if (mode > e_perout_mode_inactive_flight_mode) {
        ...
        if (stickIndex < NUM_STICKS) {
          v = md->noExpo ? rawAnalogs[stickIndex] : anas[stickIndex];
        } else {
