...
void applyExpos(int16_t *analogs, uint8_t mode)
{
	int16_t analogs2[NUM_INPUTS]; // values before expo, to ensure same expo base when multiple expo lines are used
	memcpy(analogs2, analogs, sizeof(analogs2));
...
// therefore with go with this compromise, interested people could activate this define
...
int16_t applyLimits(uint8_t channel, int32_t value)
{
	...
	int16_t ofs   = calc1000toRESX(lim->offset);//LIMIT_OFS_RESX(lim);
...
void evalInputs(uint8_t mode)
{
	...
	if (g_model.swashRing.value) 
	{
		...
		uint32_t q = calc100toRESX(g_model.swashRing.value);
		q *= q;
		if (v > q) 
		{
			d = isqrt32(v);
	...
	for (uint8_t i=0; i<NUM_STICKS+NUM_POTS; i++) 
	{
		...
		int16_t v = analogIn(i);

		if (i < NUM_STICKS+NUM_POTS) 
		{			
			Calibration * calib = &g_general.calib[i];
			v -= calib->middle;
			v = v * (int32_t)RESX / (max((int16_t)100, (v > 0 ? calib->max : calib->min)));
		}
		...
		if (i < NUM_STICKS+NUM_POTS) 
		{
			...
			if (tmp <= 1) 
				anaCenter |= (tmp==0 ? mask : (beepOnCenter & mask));
		} 
		else 
		{
			// rotary encoders
		...
		{ 
			...
			if (mode <= e_perout_mode_inactive_flight_mode && modelFunctionsContext.isFunctionActive(FUNCTION_TRAINER+ch) && IS_TRAINER_INPUT_VALID()) 
			{
				// trainer mode
				TrainerChannel* td = &g_general.trainer.mix[ch];
				if (td->mode) 
				{
					uint8_t chStud = td->channel;
					int32_t vStud  = (ppmInput[chStud]- g_general.trainer.calib[chStud]);
					vStud *= td->weight;
					vStud /= 50;
					switch (td->mode) 
					{
			...
			#endif
			rawAnalogs[ch] = v;
			analogs[ch] = v; // set values for mixer
		}
	}

	/* EXPOs */
	applyExpos(analogs, mode);
	...
	{
		anaCenter &= g_model.beepOnCenter;
		if (((beepOnCenter ^ anaCenter) & anaCenter))
			AUDIO_POT_MIDDLE();
		beepOnCenter = anaCenter;
	}
}
...
#define REZ_SWASH_Y(x)  ((x))   //  1024 => 1024
	...
	if (g_model.swashRing.type) 
	{
		getvalue_t vp = heliEleValue + trims[ELE_STICK];
		getvalue_t vr = heliAilValue + trims[AIL_STICK];
		getvalue_t vc = 0;
		if (g_model.swashRing.collectiveSource)
			vc = getValue(g_model.swashRing.collectiveSource);

		if (g_model.swashRing.invertELE) vp = -vp;
		if (g_model.swashRing.invertAIL) vr = -vr;
		if (g_model.swashRing.invertCOL) vc = -vc;
		...
		switch (g_model.swashR.type)
		{
			case SWASH_TYPE_120:
				...
				vr = REZ_SWASH_X(vr);
				cyc_analogs[0] = vc - vp;
				cyc_analogs[1] = vc + vp/2 + vr;
				cyc_analogs[2] = vc + vp/2 - vr;
				break;
			case SWASH_TYPE_120X:
				vp = REZ_SWASH_X(vp);
				vr = REZ_SWASH_Y(vr);
				cyc_analogs[0] = vc - vr;
				cyc_analogs[1] = vc + vr/2 + vp;
				cyc_analogs[2] = vc + vr/2 - vp;
				break;
			case SWASH_TYPE_140:
				vp = REZ_SWASH_Y(vp);
				vr = REZ_SWASH_Y(vr);
				cyc_analogs[0] = vc - vp;
				cyc_analogs[1] = vc + vp + vr;
				cyc_analogs[2] = vc + vp - vr;
				break;
			case SWASH_TYPE_90:
				vp = REZ_SWASH_Y(vp);
				vr = REZ_SWASH_Y(vr);
	...
	uint16_t dirtyChannels = (uint16_t)-1; // all dirty when mixer starts

	do {

		uint16_t passDirtyChannels = 0;
		...
		for (uint8_t i=0; i<MAX_MIXERS; i++) 
		{
			...
			if (!(dirtyChannels & ((uint16_t)1 << md->destCh))) 
				continue;


			//========== PHASE && SWITCH ==========
			uint8_t mixCondition = (md->flightModes != 0 || md->swtch);
			int8_t mixEnabled = (!(md->flightModes & (1 << mixerCurrentFlightMode)) && getSwitch(md->swtch)) ? 1 : 0;
	...
	if (mode > e_perout_mode_inactive_flight_mode) 
	{
		...
		{
			if (!(mode & e_perout_mode_nosticks)) v = analogs[stickIndex];
		}
	}
	else 
	{
		...
		{
			v = md->noExpo ? rawAnalogs[stickIndex] : analogs[stickIndex];
		}
		else
		{
			uint8_t srcRaw = MIXSRC_Rud + stickIndex;
			v = getValue(srcRaw);
			srcRaw -= MIXSRC_CH1;
