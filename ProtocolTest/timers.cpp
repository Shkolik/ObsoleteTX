...
{
	timersStates[idx].state = TMR_OFF; // is changed to RUNNING depends from mode
...
// TODO: Timers should be refined and checked for correct timings
void evalTimers(int16_t throttle, uint8_t tick10ms)
{
	for (uint8_t i=0; i<TIMERS; i++) 
	{
		int8_t timerMode = g_model.timers[i].mode;
		uint16_t timerStart = g_model.timers[i].start;
		TimerState * timerState = &timersStates[i];
		...
		if (timerMode) 
		{
			...
			{
				if (timerState->val == TIMER_MAX) 
					break;
				if (timerState->val == TIMER_MIN) 
					break;

				timerState->val_10ms -= 100 ;
				int16_t newTimerVal = timerState->val;
				if (timerStart) 
					newTimerVal = timerStart - newTimerVal;
				...
				else if (timerMode == TMRMODE_THR_REL) 
				{
					...
					{  // throttle was normalized to 0 to 128 value (throttle/64*2 (because - range is added as well)
				...
				{
					case TMR_RUNNING:
						if (timerStart && newTimerVal>=(int16_t)timerStart) 
						{
						...
						break;
					case TMR_NEGATIVE:
						if (newTimerVal >= (int16_t)timerStart + MAX_ALERT_TIME) 
						{
				...
				if (timerStart) 
					newTimerVal = timerStart - newTimerVal; // if counting backwards - display backwards
